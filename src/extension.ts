...
import { RangeNavigator, OccurrenceInfo } from './types/types';
...
import { RangeNavigatorProvider } from './providers/RangeNavigatorProvider';
import {
    initializeHighlightDecorations,
    highlightSelectedLine,
    highlightSelection,
    clearHighlights,
    disposeDecorations,
    getCurrentHighlightRange,
    getCurrentHighlightLineContent,
    getHighlightDecorationType
} from './utils/highlightUtils';
import {
    findOccurrencesInStructure,
    updateSearchContext,
    updateOccurrenceSelection,
    updateSidebarSelectionFromEditor
...
export function activate(context: vscode.ExtensionContext) {
    ...
    // プロバイダーとツリービューの初期化
    const rangeNavigatorProvider = new RangeNavigatorProvider(context, isSearchHistoryMode);
 ...
 * @param context 拡張機能のコンテキスト
 */
function loadSearchHistory(context: vscode.ExtensionContext): void {
...
function showSearchHistoryOnly(provider: RangeNavigatorProvider): void {
    ...
    if (searchHistory.length === 0) {
        ...
        for (const item of searchHistory) {
            const historyItem = new SearchHistoryItemNode(item, true);
            // 明示的に選択状態をfalseに設定する
    ...
    if (!selection.isEmpty) {
        ...
        if (selectedText && selectedText.length > 0) {
            ...
            if (autoShowSidebarOnSearch) {
                ...
                // 選択されたテキストを検索
                selectedOccurrence = await findOccurrencesInStructure(
                    editor,
                    selectedText,
                    provider,
                    null,
        ...
        editor.setDecorations(getHighlightDecorationType(), []);
 ...
 * @param treeView ツリービュー
 */
async function handleTreeViewVisibilityChange(
...
async function handleDocumentChange(
    ...
    // ハイライトが存在する場合の処理
    const currentHighlightRange = getCurrentHighlightRange();
    const currentHighlightLineContent = getCurrentHighlightLineContent();
    ...
    if (currentHighlightRange !== null && currentHighlightLineContent !== null) {
        ...
        if (foundLine !== -1) {
            ...
            editor.setDecorations(getHighlightDecorationType(), [updatedRange]);
 ...
 * @param provider ツリービュープロバイダー
 */
function registerGotoHistoryLineCommand(
    context: vscode.ExtensionContext,
    provider: RangeNavigatorProvider
): void {
    context.subscriptions.push(
                    ...
                    if (lineNumber >= 0 && lineNumber < document.lineCount) {
                        ...
                        if (currentLineText.includes(searchText)) {
                            ...
                            setTimeout(() => {
                                ...
                                findOccurrencesInStructure(editor, searchText, provider)
                                    .then(() => {
                                        // 検索結果が表示された後、該当行を選択状態にする
                                        setTimeout(() => {
                                            // 行に対応するTextOccurrenceを見つけて選択状態にする処理
                                            selectedOccurrence = updateOccurrenceSelection(
                                                provider,
                                                lineNumber,
                                                currentIndex,
                                                searchText,
 ...
 * @param provider ツリービュープロバイダー
 */
function registerShowSearchHistoryCommand(
    context: vscode.ExtensionContext,
    provider: RangeNavigatorProvider
): void {
    context.subscriptions.push(
            ...
            if (!isSearchHistoryMode) {
                ...
                if (lastSearchedText) {
                    ...
                    if (editor) {
                        ...
                        findOccurrencesInStructure(editor, lastSearchedText, provider);
                    }
                } else {
 ...
 * @param provider ツリービュープロバイダー
 */
function registerSearchAgainCommand(
    context: vscode.ExtensionContext,
    provider: RangeNavigatorProvider
): void {
    context.subscriptions.push(
        vscode.commands.registerCommand('rangeNavigator.searchAgain',
            async (searchText: string) => {
                ...
                // 選択テキストを設定して検索を実行
                lastSearchedText = searchText;
                selectedOccurrence = await findOccurrencesInStructure(editor, searchText, provider);
            }
        )
    );
}
 ...
 * @param context 拡張機能のコンテキスト
 */
function registerCollapseAllCommand(context: vscode.ExtensionContext): void {
    context.subscriptions.push(
 ...
 * @param provider ツリービュープロバイダー
 */
function registerExpandAllCommand(
    context: vscode.ExtensionContext,
    treeView: vscode.TreeView<any>,
    provider: RangeNavigatorProvider
): void {
 ...
 * @param provider ツリービュープロバイダー
 */
function registerGotoOccurrenceCommand(
    context: vscode.ExtensionContext,
    provider: RangeNavigatorProvider
): void {
    context.subscriptions.push(
 ...
 * @param context 拡張機能のコンテキスト
 */
function registerGotoDefinitionCommand(context: vscode.ExtensionContext): void {
    context.subscriptions.push(
 ...
 * @param provider ツリービュープロバイダー
 */
function registerClearSearchCommand(
    context: vscode.ExtensionContext,
    provider: RangeNavigatorProvider
): void {
    context.subscriptions.push(
 ...
 * @param treeView ツリービュー
 */
function registerFindOccurrencesCommand(
    context: vscode.ExtensionContext,
    provider: RangeNavigatorProvider,
    treeView: vscode.TreeView<any>
): void {
 ...
 * @param context 拡張機能のコンテキスト
 */
function registerCopyMessageCommand(context: vscode.ExtensionContext): void {
    context.subscriptions.push(
 ...
 * @param context 拡張機能のコンテキスト
 */
function registerClearHighlightsOnlyCommand(context: vscode.ExtensionContext): void {
    context.subscriptions.push(
