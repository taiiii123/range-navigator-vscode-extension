...
function activate(context) {
    ...
    // 検索モード切替（履歴/通常）コマンド
    context.subscriptions.push(vscode_1.default.commands.registerCommand('range-navigator.toggleSearchMode', () => {
    ...
    // 履歴モードに切り替えるコマンド
    context.subscriptions.push(vscode_1.default.commands.registerCommand('range-navigator.toggleHistoryMode', () => {
        // 履歴モードがオフの場合にオンにする
        if (!isSearchHistoryMode) {
            // 現在の選択状態を保存
            const currentSelection = selectedOccurrence;
            ...
            vscode_1.default.commands.executeCommand('rangeNavigatorView.focus');
        }
    }));
    // 通常モードに戻るコマンド
    context.subscriptions.push(vscode_1.default.commands.registerCommand('range-navigator.switchToNormalMode', () => {
        // 履歴モードがオンの場合にオフにする
        if (isSearchHistoryMode) {
            // 通常モードに切り替え
            isSearchHistoryMode = false;
