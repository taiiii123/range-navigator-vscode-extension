...
class SearchHistoryNode extends TreeNode {
    constructor() {
        const historyTitle = vscode_1.l10n.t('検索履歴:');
        ...
        this.tooltip = vscode_1.l10n.t('過去の検索を表示して再利用');
...
async function parsePythonStructure(document, text) {
    const structures = [];
    ...
    // スタック（現在のインデントコンテキスト）
    // [インデントレベル, 親ノード情報]
    let currentContext = [0, undefined];
    // 1回目のパス: ノード構造を構築
    for (let i = 0; i < lines.length; i++) {
        ...
        if (classMatch) {
            ...
            const startPos = new vscode_1.default.Position(i, 0);
            // 仮の終了位置（後で更新）
            const endPos = new vscode_1.default.Position(i + 1, 0);
            const classNode = new CodeStructureNode(className, 'class', new vscode_1.default.Range(startPos, endPos), document);
            const nodeInfo = {
                node: classNode,
        ...
        if (methodMatch && currentContext[1] && currentContext[1].node.type === 'class') {
            ...
            // 仮の終了位置
            const endPos = new vscode_1.default.Position(i + 1, 0);
            const methodNode = new CodeStructureNode(methodName, 'method', new vscode_1.default.Range(startPos, endPos), document);
            const nodeInfo = {
                node: methodNode,
                indentLevel: indentLevel,
                startLine: i,
        ...
        if (functionMatch && (!currentContext[1] || currentContext[1].node.type !== 'class')) {
            ...
            // 仮の終了位置
            const endPos = new vscode_1.default.Position(i + 1, 0);
            const functionNode = new CodeStructureNode(functionName, 'function', new vscode_1.default.Range(startPos, endPos), document);
            const nodeInfo = {
                node: functionNode,
                indentLevel: indentLevel,
                startLine: i,
    ...
    for (let i = 0; i < nodeInfos.length; i++) {
        ...
        const newNode = new CodeStructureNode(nodeInfo.node.name, nodeInfo.node.type, new vscode_1.default.Range(nodeInfo.node.range.start, new vscode_1.default.Position(endLine + 1, 0)), document);
        // 子ノードの追加
...
function activate(context) {
    ...
    context.subscriptions.push(vscode_1.default.commands.registerCommand('rangeNavigator.gotoHistoryLine', async (historyItem, historyItemNode) => {
        ...
        try {
            ...
            vscode_1.default.window.showErrorMessage(vscode_1.l10n.t('履歴行への移動中にエラーが発生しました。'));
            isNavigatingFromSidebar = false;
        }
    }));
    // 検索履歴を表示するコマンド
    ...
    context.subscriptions.push(vscode_1.default.commands.registerCommand('rangeNavigator.searchAgain', async (searchText) => {
        const editor = vscode_1.default.window.activeTextEditor;
        if (!editor) {
            return vscode_1.default.window.showWarningMessage(vscode_1.l10n.t('アクティブなエディタが見つかりません。'));
        }
        // 検索履歴モードを解除
        isSearchHistoryMode = false;
        vscode_1.default.commands.executeCommand('setContext', 'rangeNavigator.historyMode', false);
...
async function findOccurrencesInStructure(editor, searchText, provider, prevSelection = null // 引数追加
) {
    const document = editor.document;
    const results = [];
    // 明示的にboolean型に変換する
